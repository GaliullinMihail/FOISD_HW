...
public static class ServerResponseProvider
{
    public static ResponseInfo GetResponse(string path, HttpListenerRequest request)
    {
        ...
        if (TryGetFile(path + rawUrl.Replace("%20", " "), out var buffer))
            return new ResponseInfo(buffer, GetContentType(rawUrl), HttpStatusCode.OK);

        if (TryHandleController(request, out buffer))
        {
            var statusCode = buffer.Length > 0 ? HttpStatusCode.OK : HttpStatusCode.Redirect;
            return new ResponseInfo(buffer, "application/json", statusCode);
        }
        
        buffer = Encoding.UTF8.GetBytes("Error 404. Not Found.");
        return new ResponseInfo(buffer, "text/plain", HttpStatusCode.NotFound);
    }
